...
{
    SpiceDouble start_date = TimeConverter::from_string_to_tdb(PropertiesReader::get_property("forward-task", "simulation", "start-date"))/day;
    SpiceDouble step = TimeConverter::from_string_to_tdb(PropertiesReader::get_property("forward-task", "simulation", "step"))/day;
    SpiceDouble end_date = TimeConverter::from_string_to_tdb(PropertiesReader::get_property("forward-task", "simulation", "end-date"))/day;
    std::cout << std::fixed << std::setprecision(0) << start_date << " " << step << " " << end_date << std::endl;
    ExtendedBodyState start_state( // mercury
    ...
    NewtonFormula *newton = new NewtonFormula( "MESSENGER", start_state, step);
